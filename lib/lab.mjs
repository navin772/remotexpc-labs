...
terminal and leave it running:
  ...
  ${C.bold}sudo appium driver run xcuitest tunnel-creation${C.off}
...
export async function run(fn) {
  try {
    ...
    failed++;
    console.log(`\n${C.bad}✗ ${err?.message ?? err}${C.off}`);
    if (/tunnel|registry|ECONNREFUSED|not available|no such/i.test(String(err?.message ?? err))) {
      tunnelHint();
